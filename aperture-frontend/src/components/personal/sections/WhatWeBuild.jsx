    ...
    <section className="relative px-6 py-24 md:px-12 md:py-32">
      <div className="container mx-auto">
        <div className="mb-14 max-w-2xl">
          <Reveal>
            <span className="eyebrow text-white/50">What We Build</span>
          </Reveal>
          <AnimatedHeading
            as="h2"
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {whatWeBuild.map((card, i) => (
            <motion.div
              ...
              whileHover={{ y: -6 }}
              className="group relative overflow-hidden rounded-2xl dark-glass p-7"
            >
              <span
              ...
              <h3 className="mt-8 font-display text-2xl font-medium text-white">{card.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-white/50">{card.blurb}</p>
            </motion.div>
          ))}
        </div>
      </div>
