  ...
  return (
    <section className="relative px-6 py-24 md:px-12 md:py-32">
      <div className="container mx-auto grid grid-cols-1 items-center gap-14 lg:grid-cols-2">
        <div>
          <Reveal>
            <span className="eyebrow text-cocoa-soft">Digital Business Card</span>
          </Reveal>
          <AnimatedHeading
            as="h2"
            ...
            >
              ...
              >
                ...
                <div className="relative">
                  <h3 className="font-display text-4xl font-medium text-cream">Sanjeev Sriram</h3>
              ...
              {/* Back */}
              <div
                className="absolute inset-0 grid grid-cols-2 gap-3 rounded-2xl bg-cream p-6 [backface-visibility:hidden] [transform:rotateY(180deg)]"
              >
                {links.map((l) => (
                  <div
                    key={l.label}
                    className="flex flex-col justify-center gap-1 rounded-xl border border-white/60 bg-white/60 p-4"
                  >
                    <l.icon className="h-4 w-4 text-gold" />
                    <span className="text-xs font-medium text-cocoa">{l.label}</span>
                    <span className="text-[11px] text-cocoa-soft">{l.value}</span>
                  </div>
                ))}
              </div>
            </motion.div>
